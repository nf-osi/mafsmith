#!/usr/bin/env python3
"""
vcf_diff.py — run mafsmith+fastVEP and vcf2maf.pl+VEP on a VCF and
produce a structured diff for edge-case analysis.

Input: a Synapse entity ID (auto-downloaded) or a local VCF / VCF.gz path.
Both pipelines run with full annotation at maximum parallelism:
  mafsmith:   Rayon (all cores), fastVEP runs internally
  vcf2maf.pl: --vep-forks 16 (overridable with --vep-forks N)
...
Outputs (saved to --output-dir, default /tmp/vcf_diff_<id>/):
    mafsmith.maf    mafsmith + fastVEP MAF
    vcf2maf.maf     vcf2maf.pl + VEP 115 MAF
...
def main():
    ...
    try:
        ...
        if not args.no_vcf2maf and vcf2maf:
            ...
            run_desc = (
                f"mafsmith+fastVEP (all cores) vs vcf2maf.pl+VEP 115 "
                f"(--vep-forks {args.vep_forks})"
